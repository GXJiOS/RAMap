...
export function YRMPreviewTool() {
  ...
  return <ToolWorkspace title="红警地图预览" description="载入红警地图，预览缩略图或按游戏版本编辑矿石与宝石。" error={error} status={status}>
        ...
        <div className="yrm-detail-layout">
          <Panel title="地图预览" actions={<>
            ...
            <div className={`yrm-stage ${zoom === 'fit' ? 'is-fit' : 'is-zoomed'}`}>
              {selected?.image ? <img alt={`${selected.filename} 地图预览`} src={selected.image.url} draggable={false}
                style={zoom === 'fit' ? undefined : { width: selected.image.width * Number(zoom), height: selected.image.height * Number(zoom) }} />
            ...
            <p className="yrm-preview-note">显示地图内嵌缩略图，清晰度取决于原图。实际地形与可玩性以游戏为准。</p>
          </Panel>
